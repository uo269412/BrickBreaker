...
bool Actor::isAbove(Actor* actor) {
	bool above = false;
	if (isOverlap(actor) && actor->y > y) {
		above = true;
	}
	return above;
}

bool Actor::verticallyHits(Actor* actor) {
	bool overlap = false;
	if (actor->y + actor->height / 2 >= y - height / 2
		&& actor->y - actor->height / 2 <= y + height / 2) {
...
bool Actor::horizontallyHits(Actor* actor) {
	bool overlap = false;
